...
std::vector<std::string> MemoryManager::load_memory() const {
  ...
  if (!file.is_open()) {
    throw std::runtime_error("Unable to open memory file for reading: " +
                             memory_file_);
  }
...
std::string MemoryManager::get_context_string() const {
  ...
  size_t start_index =
      memory.size() > max_lines ? memory.size() - max_lines : 0;
...
size_t MemoryManager::get_memory_size() const { return load_memory().size(); }
