...
namespace Services {
    ...
    std::string ThemeService::get_ansi_color_code(const std::string& color) {
        ...
        if (color == "#00FFFF") return "\033[96m"; // Bright cyan
        if (color == "#FF00FF") return "\033[95m"; // Bright magenta
        if (color == "#00FF00") return "\033[92m"; // Bright green
        if (color == "#FFFF00") return "\033[93m"; // Bright yellow
        if (color == "#FF0080") return "\033[91m"; // Bright red
        if (color == "#4A9EFF") return "\033[94m"; // Bright blue
        if (color == "#40E0D0") return "\033[96m"; // Turquoise
        if (color == "#FFD700") return "\033[93m"; // Gold
        if (color == "#FF6B6B") return "\033[91m"; // Light red
        if (color == "#64B5F6") return "\033[94m"; // Light blue
        if (color == "#BB86FC") return "\033[95m"; // Light purple
        if (color == "#28A745") return "\033[32m"; // Green
        if (color == "#DC3545") return "\033[31m"; // Red
        if (color == "#FFC107") return "\033[33m"; // Yellow
        if (color == "#17A2B8") return "\033[36m"; // Cyan
        if (color == "#007BFF") return "\033[34m"; // Blue
        if (color == "#0066CC") return "\033[34m"; // Blue
        if (color == "#006994") return "\033[34m"; // Dark blue
        if (color == "#2E8B57") return "\033[32m"; // Sea green
        if (color == "#20B2AA") return "\033[36m"; // Light sea green
    ...
    std::string ThemeService::get_current_theme() {
        ...
        try {
            ...
            for (const auto& [name, theme] : available_themes_) {
                ...
                for (const auto& [color_type, color_value] : theme.colors) {
                    ...
                    switch (color_type) {
                        case ThemeColor::PRIMARY: color_key = "primary"; break;
                        case ThemeColor::SECONDARY: color_key = "secondary"; break;
                        case ThemeColor::SUCCESS: color_key = "success"; break;
                        case ThemeColor::WARNING: color_key = "warning"; break;
                        case ThemeColor::ERROR: color_key = "error"; break;
                        case ThemeColor::INFO: color_key = "info"; break;
                        case ThemeColor::TEXT: color_key = "text"; break;
                        case ThemeColor::BACKGROUND: color_key = "background"; break;
                        case ThemeColor::ACCENT: color_key = "accent"; break;
                        case ThemeColor::MUTED: color_key = "muted"; break;
                    }
        ...
        try {
            ...
            if (config.contains("themes")) {
                for (const auto& [name, theme_json] : config["themes"].items()) {
                    ...
                    if (name == "default" || name == "dark" || name == "cyberpunk" || name == "ocean") {
                        continue;
                    }
                    ...
                    if (theme_json.contains("colors")) {
                        for (const auto& [color_key, color_value] : theme_json["colors"].items()) {
                            ThemeColor color_type;
                            if (color_key == "primary") color_type = ThemeColor::PRIMARY;
                            else if (color_key == "secondary") color_type = ThemeColor::SECONDARY;
                            else if (color_key == "success") color_type = ThemeColor::SUCCESS;
                            else if (color_key == "warning") color_type = ThemeColor::WARNING;
                            else if (color_key == "error") color_type = ThemeColor::ERROR;
                            else if (color_key == "info") color_type = ThemeColor::INFO;
                            else if (color_key == "text") color_type = ThemeColor::TEXT;
                            else if (color_key == "background") color_type = ThemeColor::BACKGROUND;
                            else if (color_key == "accent") color_type = ThemeColor::ACCENT;
                            else if (color_key == "muted") color_type = ThemeColor::MUTED;
                            else continue;
    ...
    bool ThemeService::create_custom_theme(const std::string& name, const Theme& theme) {
        available_themes_[name] = theme;
    ...
    void ThemeService::print_theme_preview(const std::string& theme_name) {
        ...
        std::cout << "Type: " << (theme.is_dark_theme ? "Dark" : "Light") << std::endl;
        std::cout << std::endl;
        
        std::cout << "Color Preview:" << std::endl;
        std::cout << "  " << colorize("Primary", ThemeColor::PRIMARY) << " - Commands and headers" << std::endl;
        std::cout << "  " << colorize("Success", ThemeColor::SUCCESS) << " - Success messages" << std::endl;
        std::cout << "  " << colorize("Warning", ThemeColor::WARNING) << " - Warning messages" << std::endl;
        std::cout << "  " << colorize("Error", ThemeColor::ERROR) << " - Error messages" << std::endl;
        std::cout << "  " << colorize("Info", ThemeColor::INFO) << " - Information messages" << std::endl;
        std::cout << "  " << colorize("Accent", ThemeColor::ACCENT) << " - Highlights and accents" << std::endl;
    }
    
    void ThemeService::reset_colors() {
