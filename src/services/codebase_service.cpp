...
std::vector<std::string> CodebaseService::find_todos(const std::string& path) {
    ...
    auto search_results = FileService::search_in_directory(path, "TODO|FIXME|HACK", "*");
...
std::string CodebaseService::get_directory_tree(const std::string& path, int max_depth) {
    std::ostringstream tree;
    build_tree_recursive(path, tree, "", 0, max_depth);
    return tree.str();
}

void CodebaseService::build_tree_recursive(const std::string& path, std::ostringstream& tree, 
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
        ...
        if (entry.is_directory()) {
            ...
            build_tree_recursive(entry.path().string(), tree, new_prefix, depth + 1, max_depth);
        } else {
            tree << "\n";
        }
    }
...
std::map<std::string, int> CodebaseService::analyze_file_types(const std::string& path) {
...
std::vector<std::string> CodebaseService::find_key_files(const std::string& path) {
    std::vector<std::string> key_files;
    
    // Patterns for important files
    std::vector<std::regex> patterns = {
        std::regex(".*main\\.(cpp|c|py|js|ts|java)$", std::regex_constants::icase),
        std::regex(".*index\\.(js|ts|html|php)$", std::regex_constants::icase),
        std::regex(".*app\\.(cpp|c|py|js|ts|java)$", std::regex_constants::icase),
        std::regex(".*server\\.(cpp|c|py|js|ts|java)$", std::regex_constants::icase),
        std::regex(".*client\\.(cpp|c|py|js|ts|java)$", std::regex_constants::icase)
    };
