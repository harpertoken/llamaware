...
bool AIService::is_online_mode() const {
  return mode_ == Core::Agent::MODE_TOGETHER ||
         mode_ == Core::Agent::MODE_CEREBRAS ||
         mode_ == Core::Agent::MODE_FIREWORKS ||
         mode_ == Core::Agent::MODE_GROQ ||
         mode_ == Core::Agent::MODE_DEEPSEEK ||
         mode_ == Core::Agent::MODE_OPENAI;
}
...
std::string AIService::chat(const std::string &user_input,
  ...
  try {
    ...
    if (mode_ == Core::Agent::MODE_TOGETHER ||
        mode_ == Core::Agent::MODE_CEREBRAS ||
        mode_ == Core::Agent::MODE_FIREWORKS ||
        mode_ == Core::Agent::MODE_GROQ ||
        mode_ == Core::Agent::MODE_DEEPSEEK ||
        mode_ == Core::Agent::MODE_OPENAI ||
        mode_ == Core::Agent::MODE_LLAMA_3B ||
        mode_ == Core::Agent::MODE_LLAMA_LATEST ||
