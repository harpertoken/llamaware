...
namespace Services {
    ...
    std::string FileService::read_file_range(const std::string& filename, int start_line, int line_count) {
        try {
            if (!file_exists(filename)) {
                return "Error: File '" + filename + "' does not exist";
            }
            ...
            int end_line = (line_count == -1) ? static_cast<int>(lines.size()) : std::min(start_line + line_count, static_cast<int>(lines.size()));
            
            std::ostringstream result;
            for (int i = start_line; i < end_line; ++i) {
                result << lines[i];
                if (i < end_line - 1) result << "\n";
            }
    ...
    FileEditResult FileService::replace_text_in_file(const std::string& filename, 
        ...
        try {
            ...
            if (expected_replacements > 0 && count != expected_replacements) {
                result.message = "Error: Expected " + std::to_string(expected_replacements) + 
    ...
    std::vector<FileSearchResult> FileService::search_in_file(const std::string& filename, 
    ...
    std::vector<FileSearchResult> FileService::search_in_directory(const std::string& directory, 
                                                                  const std::string& pattern,
    ...
    std::string FileService::get_relative_path(const std::string& filepath, const std::string& base_path) {
        try {
    ...
    bool FileService::is_within_directory(const std::string& filepath, const std::string& base_directory) {
        try {
            std::filesystem::path file_path = std::filesystem::absolute(filepath);
