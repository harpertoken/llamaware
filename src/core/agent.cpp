...
namespace Core {
    ...
    void Agent::handle_direct_command(const std::string& input) {
        ...
        if (input.rfind("search:", 0) == 0) {
            ...
            if (first_colon != std::string::npos) {
                ...
                if (!validation.is_valid) {
                    ...
                    if (second_colon != std::string::npos) {
                        ...
                        result = Services::FileService::read_file_range(filename, start_line, line_count);
                    } else {
            ...
            while ((colon_pos = params.find(':', pos)) != std::string::npos && parts.size() < 3) {
            ...
            if (parts.size() >= 3) {
                ...
                auto edit_result = Services::FileService::replace_text_in_file(filename, old_text, new_text, expected);
            ...
            while ((colon_pos = params.find(':', pos)) != std::string::npos && parts.size() < 2) {
                parts.push_back(params.substr(pos, colon_pos - pos));
                pos = colon_pos + 1;
            }
            ...
            auto search_results = Services::FileService::search_in_directory(directory, pattern, filter);
            ...
            result = global_context.empty() ? "No global memories stored" : global_context;
        }
    ...
    void Agent::handle_ai_chat(const std::string& input) {
        ...
        std::string hierarchical_context = Services::ContextService::load_hierarchical_context(".");
    ...
    std::string Agent::read_file_or_directory(const std::string& path) {
        try {
            ...
            if (std::filesystem::is_directory(path)) {
                ...
                auto files = Services::MultiFileService::read_directory_files(path, options);
    ...
    void Agent::toggle_shell_mode() {
        ...
        if (shell_mode_) {
            std::cout << "Entering shell mode. Type commands directly or '!' to exit." << std::endl;
        } else {
    ...
    void Agent::show_meta_help() {
        ...
        std::cout << "  /context create       - Create LLAMAWARE.md file" << std::endl;
    ...
    void Agent::compress_context() {
        try {
            std::cout << "Analyzing conversation context for compression..." << std::endl;
            ...
            if (compressible_content.empty()) {
                std::cout << "Not enough conversation history to compress (need at least 5 interactions)." << std::endl;
                return;
            }
            ...
            std::string compression_prompt = 
                ...
                "Preserve key information, decisions made, important facts discovered, and context that would be needed for future interactions. "
                "Focus on actionable information and maintain continuity. Keep the summary under 500 words.\n\n"
            ...
            std::cout << "Original interactions have been summarized and recent context preserved." << std::endl;
    ...
    void Agent::handle_context_management(const std::string& command) {
        if (command == "show") {
            std::string context = Services::ContextService::load_hierarchical_context(".");
            if (context.empty()) {
                std::cout << "No hierarchical context found." << std::endl;
                std::cout << "Create a LLAMAWARE.md file with /context create" << std::endl;
            } else {
            ...
            if (Services::ContextService::create_context_file(".")) {
                ...
                std::cout << "Could not create LLAMAWARE.md (file may already exist)." << std::endl;
            }
        } else {
    ...
    void Agent::handle_multi_file_command(const std::string& command) {
        if (command.empty()) {
            std::cout << "Usage: /files <path1> [path2] [--include pattern] [--exclude pattern]" << std::endl;
    ...
    void Agent::handle_web_fetch_command(const std::string& command) {
        if (command.empty()) {
            ...
            std::cout << "Examples:" << std::endl;
            std::cout << "  /fetch https://api.github.com/users/octocat json" << std::endl;
        ...
        if (!Services::WebService::is_valid_url(url)) {
            std::cout << "Error: Invalid URL format. Use http:// or https://" << std::endl;
            return;
        }
    ...
    void Agent::handle_checkpoint_command(const std::string& command) {
        if (command.empty() || command == "help") {
            ...
            std::cout << "  /checkpoint create <name> [description] - Create new checkpoint" << std::endl;
            std::cout << "  /checkpoint list                       - List all checkpoints" << std::endl;
            std::cout << "  /checkpoint info <id>                  - Show checkpoint details" << std::endl;
            std::cout << "  /checkpoint delete <id>                - Delete checkpoint" << std::endl;
            std::cout << "  /checkpoint cleanup [count]            - Keep only N recent checkpoints" << std::endl;
            std::cout << "  /restore                               - List checkpoints for restore" << std::endl;
            std::cout << "  /restore <id>                          - Restore from checkpoint" << std::endl;
            return;
        }
        ...
        try {
            if (action == "create") {
                ...
                if (name.empty()) {
                    ...
                    std::cout << "Usage: /checkpoint create <name> [description]" << std::endl;
                    return;
                }

                std::cout << "Creating checkpoint '" << name << "'..." << std::endl;
                std::string checkpoint_id = Services::CheckpointService::create_checkpoint(name, description);
                std::cout << "✓ Checkpoint created with ID: " << checkpoint_id << std::endl;
                ...
                auto info = Services::CheckpointService::get_checkpoint_info(checkpoint_id);
                ...
                std::cout << "  Total Size: " << (info.total_size / 1024) << " KB" << std::endl;
                std::cout << "  Files (" << info.backed_up_files.size() << "):" << std::endl;
                ...
                if (Services::CheckpointService::delete_checkpoint(checkpoint_id)) {
                    ...
                    std::cout << "Error: Failed to delete checkpoint " << checkpoint_id << std::endl;
                }

            } else if (action == "cleanup") {
                int keep_count = 10;
                iss >> keep_count;
                if (keep_count < 1) keep_count = 10;

                Services::CheckpointService::cleanup_old_checkpoints(keep_count);
                std::cout << "✓ Cleaned up old checkpoints, keeping " << keep_count << " most recent" << std::endl;
                ...
                if (checkpoint_id.empty()) {
                    ...
                    for (const auto& cp : checkpoints) {
                        std::cout << "  " << cp.id << " - " << cp.name << " (" << cp.timestamp << ")" << std::endl;
                    }
                ...
                std::cout << "Note: A backup will be created automatically before restore." << std::endl;
                ...
                if (Services::CheckpointService::restore_checkpoint(checkpoint_id, options)) {
                    std::cout << "✓ Successfully restored from checkpoint " << checkpoint_id << std::endl;
                    std::cout << "Note: You may need to restart the application to see all changes." << std::endl;
                } else {
                    std::cout << "Error: Failed to restore from checkpoint " << checkpoint_id << std::endl;
                }

            } else {
                std::cout << "Unknown checkpoint command: " << action << std::endl;
                std::cout << "Use '/checkpoint help' for available commands." << std::endl;
            }
        ...
        // Save to memory for AI context
        memory_->save_interaction("/checkpoint " + command, "Checkpoint command executed");
    }

    void Agent::handle_mcp_command(const std::string& command) {
        if (command.empty() || command == "help") {
            std::cout << "MCP (Model Context Protocol) commands:" << std::endl;
            std::cout << "  /mcp servers                           - List registered MCP servers" << std::endl;
            std::cout << "  /mcp status <server>                   - Show server status" << std::endl;
            std::cout << "  /mcp start <server>                    - Start MCP server" << std::endl;
            std::cout << "  /mcp stop <server>                     - Stop MCP server" << std::endl;
            std::cout << "  /mcp resources <server>                - List server resources" << std::endl;
            std::cout << "  /mcp read <server> <uri>               - Read resource from server" << std::endl;
            std::cout << "  /mcp tools <server>                    - List server tools" << std::endl;
            std::cout << "  /mcp call <server> <tool> [args]       - Call server tool" << std::endl;
            std::cout << "  /mcp prompts <server>                  - List server prompts" << std::endl;
            std::cout << "  /mcp prompt <server> <name> [args]     - Get server prompt" << std::endl;
            return;
        }
        ...
        try {
            ...
            if (action == "servers") {
                ...
                if (servers.empty()) {
                    ...
                    std::cout << "Default servers (filesystem, git) will be added automatically." << std::endl;
                    return;
                }
                ...
                std::string status = Services::MCPService::get_server_status(server_name);
                std::cout << "Server '" << server_name << "' status: " << status << std::endl;
                ...
                std::cout << "Starting MCP server '" << server_name << "'..." << std::endl;
                if (Services::MCPService::is_server_running(server_name)) {
                    std::cout << "Server is already running." << std::endl;
                } else {
                    // This would start the actual server in a full implementation
                    std::cout << "✓ MCP server '" << server_name << "' started (simulated)" << std::endl;
                    std::cout << "Note: Full MCP server integration requires process management implementation." << std::endl;
                }
                ...
                std::cout << "Stopping MCP server '" << server_name << "'..." << std::endl;
                std::cout << "✓ MCP server '" << server_name << "' stopped (simulated)" << std::endl;
                ...
                if (resources.empty()) {
                    std::cout << "No resources available from server '" << server_name << "'" << std::endl;
                    return;
                }

                std::cout << "Resources from server '" << server_name << "':" << std::endl;
                ...
                std::cout << "Reading resource '" << uri << "' from server '" << server_name << "'..." << std::endl;
                std::string content = Services::MCPService::read_resource(server_name, uri);
                ...
                if (tools.empty()) {
                    std::cout << "No tools available from server '" << server_name << "'" << std::endl;
                    return;
                }
                ...
                if (server_name.empty() || tool_name.empty()) {
                    std::cout << "Error: Server name and tool name are required" << std::endl;
                    return;
                }
                ...
                std::cout << "Calling tool '" << tool_name << "' on server '" << server_name << "'..." << std::endl;
                auto result = Services::MCPService::call_tool(server_name, tool_name, args);
                ...
                if (prompts.empty()) {
                    std::cout << "No prompts available from server '" << server_name << "'" << std::endl;
                    return;
                }
                ...
                if (server_name.empty() || prompt_name.empty()) {
                    std::cout << "Error: Server name and prompt name are required" << std::endl;
                    return;
                }
                ...
                std::cout << "Getting prompt '" << prompt_name << "' from server '" << server_name << "'..." << std::endl;
                std::string prompt_text = Services::MCPService::get_prompt(server_name, prompt_name, args);
    ...
    void Agent::handle_theme_command(const std::string& command) {
        if (command.empty() || command == "help") {
            ...
            std::cout << "  /theme list                            - List available themes" << std::endl;
            std::cout << "  /theme set <name>                      - Set active theme" << std::endl;
            std::cout << "  /theme preview <name>                  - Preview theme colors" << std::endl;
            std::cout << "  /theme current                         - Show current theme" << std::endl;
            return;
        }
        ...
        try {
            ...
            if (action == "list") {
                ...
                for (const auto& theme_name : themes) {
                    ...
                    std::string current = (theme_name == Services::ThemeService::get_current_theme()) ? " (current)" : "";
                    std::cout << "  " << Services::ThemeService::colorize_accent(theme_name) << current << std::endl;
                ...
                if (Services::ThemeService::set_theme(theme_name)) {
                    std::cout << Services::ThemeService::colorize_success("✓ Theme set to: " + theme_name) << std::endl;
                } else {
                    std::cout << Services::ThemeService::colorize_error("Error: Theme not found: " + theme_name) << std::endl;
                }
                ...
                std::cout << "Current theme: " << Services::ThemeService::colorize_accent(current_theme) << std::endl;
    ...
    void Agent::handle_auth_command(const std::string& command) {
        if (command.empty() || command == "help") {
            ...
            std::cout << "  /auth providers                        - List available providers" << std::endl;
            std::cout << "  /auth set <provider>                   - Set active provider" << std::endl;
            std::cout << "  /auth key <provider> <key>             - Set API key for provider" << std::endl;
            std::cout << "  /auth status [provider]                - Show provider status" << std::endl;
            std::cout << "  /auth test <provider>                   - Test provider connection" << std::endl;
            return;
        }
        ...
        try {
            ...
            if (action == "providers") {
                ...
                for (const auto& provider_name : providers) {
                    auto provider_info = Services::AuthService::get_provider_info(provider_name);
                    std::string status = Services::AuthService::get_provider_status(provider_name);
                    std::string active = provider_info.is_active ? " (active)" : "";
                    std::cout << "  " << provider_info.display_name << active << " - " << status << std::endl;
                ...
                if (Services::AuthService::set_active_provider(provider_name)) {
                    ...
                    std::cout << "Error: Provider not found: " << provider_name << std::endl;
                }
                ...
                if (provider_name.empty() || api_key.empty()) {
                    std::cout << "Error: Provider name and API key are required" << std::endl;
                    return;
                }

                if (Services::AuthService::set_api_key(provider_name, api_key)) {
                    std::cout << "✓ API key set for provider: " << provider_name << std::endl;
                } else {
                    std::cout << "Error: Failed to set API key for provider: " << provider_name << std::endl;
                }
                ...
                if (Services::AuthService::test_provider_connection(provider_name)) {
                    std::cout << "✓ Connection test successful for: " << provider_name << std::endl;
                } else {
                    std::cout << "✗ Connection test failed for: " << provider_name << std::endl;
                }
    ...
    void Agent::handle_sandbox_command(const std::string& command) {
        if (command.empty() || command == "help") {
            ...
            std::cout << "  /sandbox run <command>                 - Execute command in sandbox" << std::endl;
            std::cout << "  /sandbox list                          - List available sandbox configs" << std::endl;
            std::cout << "  /sandbox status                        - Show Docker status" << std::endl;
            std::cout << "  /sandbox containers                    - List active containers" << std::endl;
            std::cout << "  /sandbox cleanup                       - Clean up old containers" << std::endl;
            return;
        }
        ...
        try {
            ...
            if (action == "run") {
                ...
                std::cout << "Execution time: " << result.execution_time_seconds << "s" << std::endl;
                ...
                for (const auto& config_name : configs) {
                    ...
                    std::cout << "    Memory: " << config.memory_limit_mb << "MB, CPU: " << config.cpu_limit_percent << "%" << std::endl;
                }

            } else if (action == "status") {
                if (Services::SandboxService::check_docker_installation()) {
                    std::cout << "Docker Status: Available" << std::endl;
                    std::cout << "Version: " << Services::SandboxService::get_docker_version() << std::endl;
                } else {
                    std::cout << "Docker Status: Not Available" << std::endl;
                    std::cout << "Please install Docker to use sandbox features." << std::endl;
                }
    ...
    void Agent::handle_error_command(const std::string& command) {
        if (command.empty() || command == "help") {
            ...
            std::cout << "  /error report                          - Show error summary" << std::endl;
            std::cout << "  /error recent [count]                  - Show recent errors" << std::endl;
            std::cout << "  /error clear                           - Clear error log" << std::endl;
            std::cout << "  /error export <path>                   - Export error log" << std::endl;
            return;
        }
        ...
        try {
            ...
            if (action == "report") {
                ...
                if (recent_errors.empty()) {
                    ...
                    std::cout << "Recent errors (" << recent_errors.size() << "):" << std::endl;
                    for (const auto& error : recent_errors) {
                        std::cout << Services::ErrorService::format_error(error, true) << std::endl << std::endl;
                    }
                }
