...
namespace Services {
    ...
    struct MultiFileOptions {
        ...
        std::vector<std::string> language_filters; // e.g., "cpp", "python", "javascript"
    };
    
    class MultiFileService {
    public:
        // Read multiple files based on patterns
        static std::vector<FileMatch> read_many_files(
            const std::vector<std::string>& paths,
            const MultiFileOptions& options = MultiFileOptions{}
        );
        
        // Read files from directory with patterns
        static std::vector<FileMatch> read_directory_files(
            const std::string& directory,
            const MultiFileOptions& options = MultiFileOptions{}
        );
        
        // Check if file should be included based on patterns
        static bool should_include_file(
        ...
        static std::set<std::string> load_gitignore_patterns(const std::string& directory);
        
        // Check if path matches gitignore patterns
        static bool matches_gitignore(
            const std::string& file_path,
            const std::set<std::string>& patterns
        );
        
        // Format multi-file content for AI
        static std::string format_multi_file_content(
            const std::vector<FileMatch>& files,
            const std::string& context_description = ""
        );
        
        // Advanced Git-aware filtering
        static std::vector<std::string> get_git_status_files(
            const std::string& directory,
            GitStatusFilter filter
        );
        
        static bool is_text_file(const std::string& file_path);
        static bool matches_language_filter(
            const std::string& file_path,
    ...
    private:
        ...
        static bool matches_pattern(const std::string& file_path, const std::string& pattern);
    };
}