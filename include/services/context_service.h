...
namespace Services {
    ...
    public:
        ...
        static std::string load_hierarchical_context(const std::string& working_directory = ".");
        
        // Find all LLAMAWARE.md files in hierarchy
        static std::vector<ContextFile> find_context_files(const std::string& working_directory = ".");
        
        // Load global context file
        static ContextFile load_global_context();
        
        // Load project context files (from root to current directory)
        static std::vector<ContextFile> load_project_context(const std::string& working_directory);
        
        // Load local context files (in current directory and subdirectories)
        static std::vector<ContextFile> load_local_context(const std::string& working_directory);
        ...
        static std::string find_project_root(const std::string& starting_directory = ".");
        ...
        static bool create_context_file(const std::string& directory, const std::string& content = "");
    ...
    private:
        ...
        static std::string get_display_path(const std::string& file_path, const std::string& base_directory);
    };
}